...
function xls2json(argv) {
  ...
  var dirName = path.resolve(argv._[2]);
    ...
    for(var C = range.s.c; C <= range.e.c; ++C) {
      ...
      if (R == 0) {
        ...
        if (C > 0) {
          ...
          if (!cell) {
            throw new Error('lang code not found: ' + JSON.stringify(cell_address));
          }
        ...
        if (C == 0 && !cell) {
          throw new Error('key not found: ' + JSON.stringify(cell_address));
        }
        if (C > 0 && !cell) {
